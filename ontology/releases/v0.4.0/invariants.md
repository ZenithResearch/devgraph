...
`Actor`, `Person`, `Agent`, and `Organization` are canonical directory and
provenance vocabulary with `runtimeLabel: false` in v0.4.0. They do not create
runtime labels, constraints, generic Work kinds, or HTTP routes.

An Actor record is not a credential or authentication Subject. A verified
transport context may resolve its `actor_id` to an Actor, but an operation
...
```bash
rg "accepted Proposal|Decision|archive|WorkRequest|Case|Subcase|direct delete|HermesSessionRef|ReadinessAssessment|Actor|Person|Agent|Organization|MEMBER_OF" ontology/invariants.md
