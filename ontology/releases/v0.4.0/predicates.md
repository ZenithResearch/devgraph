...
| Predicate | From | To | Meaning |
|---|---|---|---|
| `MEMBER_OF` | Person/Agent/Organization | Organization | Directory membership, including nested organizations; does not grant authority. |
| `OPERATED_BY` | Agent | Actor | Operational responsibility for an Agent, including an Agent delegation chain; does not delegate a credential. |
| `ASSIGNED_TO` | Todo-derived work | Actor | Actor expected to carry out work; does not authorize a mutation. |
| `OWNED_BY` | Todo-derived work | Actor | Accountable steward for work; does not assert data or credential ownership. |
| `ATTRIBUTED_TO` | Todo-derived work/Artifact/Decision/Handoff/EventReceipt/HermesSessionRef | Actor | Directory-level provenance attribution; not cryptographic proof by itself. |

`HAS_EXTERNAL_LINK` may also start from an `Actor` when the target is a
