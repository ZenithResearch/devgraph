...
`Arena` is a separate runtime type in v0.6.0 with `runtimeLabel: true`, outside
the `Todo` hierarchy. It was introduced as ontology vocabulary in v0.5.0.
...
The signed runtime enforces these transitions:
...
Proposal acceptance rule, or existing EventReceipt. Mutations produce their own
audit receipts. An Arena is not a status lane or an
...
The v0.1.0 through v0.5.0 bundles remain byte-identical. Consumers can pin an
older bundle or ignore the additive Arena terms. Existing Work records,
parent edges, operation names, and wire shapes stay valid. Migration 26 adds a
unique Arena ID constraint without rewriting Work data.

The [runtime contract](arena-runtime.md) defines stable Arena records, signed
operations, read routes, bounded traversal, version preconditions, and CLI use.
`Arena` is not added to the existing five-kind Work interface. Examples do not
authorize direct database writes.
