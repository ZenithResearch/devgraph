...
| Class | Meaning |
|---|---|
| `Entity` | Common directory identity for a named participant in work. |
| `Person` | Human Entity. |
| `Agent` | Software, AI, service, or other automated Entity. |
| `Organization` | Collective Entity such as a company, institution, project, or working group. |

Organizations are Entities because a collective entity can own, receive an
assignment, and receive attribution for work. The three concrete subclasses
are intended to be distinct categories. v0.5.0 does not publish a disjointness
...
|---|---|---|
| `MEMBER_OF` | Entity → Organization | Simple current directory membership, including nested organizations. |
| `OPERATED_BY` | Agent → Entity | Operational responsibility or an Agent delegation chain. |
| `ASSIGNED_TO` | Todo-derived work → Entity | Expected executor of work. |
| `OWNED_BY` | Todo-derived work → Entity | Accountable steward for work. |
| `ATTRIBUTED_TO` | Work/evidence/provenance → Entity | Directory-level authorship or contribution attribution. |
| `HAS_EXTERNAL_LINK` | Entity → ExternalLink | Reviewed public or non-secret external reference. |
...
A directory Entity id is an opaque reference. It is not a public key, wallet
member key, authentication Subject, credential, capability, session, or vault.
A verified transport context may later resolve its `actor_id` to an Entity, but
...
Magik verifies and routes capabilities. Devgraph owns the Entity vocabulary and
...
All four Rolodex classes have `runtimeLabel: false` in v0.5.0.
The published `Actor` identifier remains an equivalent compatibility name for
`Entity`, also with `runtimeLabel: false`. Existing v0.4.0 consumers can retain
Actor references and their original pinned bundle; no identity is reminted. This release
...
- resolution of a verified `actor_id` to an Entity;
- storage constraints, migration, and backward-compatible decoders.

## Verification

```bash
rg "Entity|Person|Agent|Organization|MEMBER_OF|OPERATED_BY|ASSIGNED_TO|OWNED_BY|ATTRIBUTED_TO" ontology/
